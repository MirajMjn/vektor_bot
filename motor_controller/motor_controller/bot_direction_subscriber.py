...
import serial
from motor_controller.kinematics import target_wheel_rpm, odometry
...
class PwmSubscriberNode(Node):
    ...
    def listener_callback(self, msg):
        ...
        if start:
            try:
                data_pico = ser.readline().decode().strip()  # Read and decode
                if data_pico:
                    print("-------------------------------")
                    print("angle", theta, wbz)
                    try:
                        rpm1, rpm2, rpm3, del_l, del_r, del_b = [float(data) for data in data_pico.split(',')] #is in pwm value

                        print(f"r1:{rpm1:.2f}\t r2:{rpm2:.2f}\t r3:{rpm3:.2f}")
                        print(f"del_l:{del_l:.2f}\tdel_r:{del_r:.2f}\tdel_b:{del_b}")
                    except (ValueError):
                    ...
                    x, y , pose = odometry(del_l, del_r, del_b)
                    print(f"x,y,pose=({x:.2f},{y:.2f},{pose:.2f})")
