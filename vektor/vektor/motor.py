...
def rpm_to_pwm(rpm, max_rpm=200, min_rpm=0, max_pwm=90, min_pwm=20):
...
class Motor:
    ...
    def rotate(self, u_n, rpm_n=0):
        ...
        if target_rpm >= 0: # don't run pid loop if u < 5, as its already insufficient to run motor
    ...
    def destroy(self, index='not_last'):
        self.pwm_in1.stop()
        self.pwm_in2.stop()
        GPIO.cleanup() # clean gpio at last to avoid "already cleaned" error
        # sys.exit(98)
